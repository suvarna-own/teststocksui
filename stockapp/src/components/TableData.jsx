...
function TableData() {
  ...
  const addToWatchlist = (stock) => {
    const exists = addwatchlist.find(
      (item) => item.symbol === stock.symbol
    );

    if (!exists) {
      setWatchlist([...addwatchlist, stock]);
    }
  };
    ...
    <>
      <main className="stock-dashboard">
        ...
        <section className="table-wrap">
          <p className='bg-red-50 px-2 py-2 width-auto'>Add to WatchList</p>
          <div className=' flex'><button className='btn btn-primary' onClick={() => addToWatchlist(stock)}>+</button><button className="btn btn-primary mx-3">-</button></div>
      ...
      <main className="my-watchlist">
        <header className="stock-header">
          <div>
            <h1>Live Stock Feed</h1>
            <p>Streaming data from <code>http://localhost:8000/watchlist/userid</code></p>
          </div>
