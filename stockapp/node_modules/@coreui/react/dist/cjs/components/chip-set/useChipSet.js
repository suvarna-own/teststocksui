...
const buildChip = (item) => {
    if (typeof item === 'string') {
        return React.createElement(CChip.CChip, { key: item, value: item }, item);
    }
    const { label, value } = item, chipProps = tslib_es6.__rest(item, ["label", "value"]);
    return React.createElement(CChip.CChip, Object.assign({ key: value, value }, chipProps), label !== null && label !== void 0 ? label : value);
};
/**
...
const useChipSet = (options) => {
    ...
    const pendingFocusValue = React.useRef(null);
    const isControlled = selected !== undefined;
    const [_selected, setSelected] = React.useState(defaultSelected);
    ...
    const getFocusableChips = () => {
        var _a, _b;
        return [
            ...((_b = (_a = rootRef.current) === null || _a === void 0 ? void 0 : _a.querySelectorAll(_const.SELECTOR_CHIP_FOCUSABLE)) !== null && _b !== void 0 ? _b : []),
        ];
    };
    ...
    const handleKeyDown = (event) => {
        ...
        const rtl = isRTL.default(rootRef.current);
        switch (event.key) {
            case 'ArrowLeft': {
    ...
    const renderChips = (children) => React.Children.map(children, (child, index) => {
        ...
        const chipValue = utils.resolveChipValue(child, index);
